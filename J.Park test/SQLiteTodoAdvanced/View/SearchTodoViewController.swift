...
//                self.reloadAction()
    ...
    // MARK: textField 데이터 segue
    @IBAction func btnSearch(_ sender: Any) {
        //segue 로 textField 데이터 보내기
        guard let searchText = tfSearch.text else { return }
                
                // Segue를 통해 TableViewController로 이동
                performSegue(withIdentifier: "SearchSegue", sender: searchText)
    }
    
    // tfSearch의 값을 tableViewController.searchText 에 할당
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        print("segue 실행됨1")
        if segue.identifier == "SearchSegue" {
            if let tableViewController = segue.destination as? TableViewController {
                if let searchText = tfSearch.text, !searchText.isEmpty {//가져온 searchtext가 nill이 아닐때
                    tableViewController.searchText = searchText
                } else {
                    tableViewController.searchText = nil//값이 없을때는 nill 값으로
                }
            }
        }
    }
